    ...
    if (which == SyscallException) {
        ...
        switch (type){
            ...
            case SC_Write:{
                ...
                // begin at writeposition
                int WrittenBytes = openfile->Write(buffer,size,writePosition);
                if(WrittenBytes == 0) printf("write file failed!\n");
                else printf("\"%s\" has wrote in file id = %d succeed!\n",buffer,fileid);
                AdvancePC();
                break;
            }
            case SC_Read:{
            ...
            case SC_Close:{
                ...
                if(!openfile){
                    ...
                    printf("File %d closed succeed!\n",fileid);
                }else printf("Failed to Clsoe File %d.\n",fileid);
                AdvancePC();
                break;
            }
#endif
